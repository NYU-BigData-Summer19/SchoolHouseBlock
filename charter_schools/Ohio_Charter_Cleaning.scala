...
// spark-shell --packages com.databricks:spark-csv_2.10:1.5.0
 ...
 spark-shell --packages com.databricks:spark-csv_2.4.0
...
val df = sqlCtx.read.format("csv").option("header", "true").option("mode", "DROPMALFORMED").load("Ohio_Charter_Clean")


SELECT PublicDistrictLocationIRN, COUNT(School)
FROM OhioCharter O1
WHERE OverallGrade2018 like 'A'
GROUP BY PublicDistrictLocationIRN
...
val dataframe_mysql = sqlCtx.read.format("jdbc").options("url","warehouse.cims.nyu.edu").option("driver","com.mysql.jdbc.Driver").option("dbtable","ac3241_SchoolHouseBlock").option("user","ac3241").option("password","bdam3bnw").load()

val data = sqlContext.read
.format("jdbc")
.option("url", "warehouse.cims.nyu.edu")
.option("user", "ac3241")
.option("password", "bdam3bnw")
.option("dbtable", "ac3241_SchoolHouseBlock")
.load()
